...
Summary: 4/8 iterations accepted, composite score improved from 0.1950 → 0.3266 (+67.5%) using a fixed list of hand-written proposals. Safety rail correctly blocked 1 proposal (similarity 0.137 < 0.30 threshold). Threshold gating correctly rejected 1 marginal improvement (+0.93%). All template variables preserved throughout.
...
python3 packages/example/src/marketing_agent/validate_optimization_loop.py

# Real LLM validation (requires GROQ_API_KEY in .env)
