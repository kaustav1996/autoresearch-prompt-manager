"""arpm-example CLI — run the marketing agent example.

Commands:
    arpm-example seed    Seed prompt templates into the API
    arpm-example run     Generate content with the marketing agent
    arpm-example loop    Run the full autoresearch optimization loop
    arpm-example status  Check API connection and prompt count
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="arpm-example",
        description="Autoresearch Prompt Manager — Marketing Agent Example",
    )
    sub = parser.add_subparsers(dest="command")

    sub.add_parser("seed", help="Seed prompt templates into the API")

    run_p = sub.add_parser("run", help="Generate content with the marketing agent")
    run_p.add_argument(
        "task",
        nargs="?",
        default="Generate a welcome email for Alice joining TechCorp",
...
async def _loop() -> None:
    ...
    from marketing_agent.demo_full_loop import main as loop_main
