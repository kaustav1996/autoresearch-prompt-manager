"""Tools that bridge the support agent to the prompt-manager client."""
...
def create_prompt_manager_tools(client: PromptManagerClient) -> list[ToolSpec]:
    """Create tools that the support agent uses to interact with prompt-manager.
    ...
    async def resolve_prompt(slug: str, session_id: str = "") -> str:
        ...
        ----------
        slug:
            The prompt slug to resolve (e.g. ``ticket-response``).
        session_id:
    ...
    async def report_metric(
        ...
        version_id:
            UUID of the specific version that produced the response.
        metric_name:
            Name of the metric (e.g. ``csat_score``, ``tone_score``).
        value:
